...
void BTree<K, V>::split_child(BTreeNode* parent, size_t child_idx)
{
    ...
    first, last
        ...
        not the element pointed by last. The function template argument InputIterator
        shall be an input iterator type that points to elements of a type from which
        value_type objects can be constructed.
...
void BTree<K, V>::insert(BTreeNode* subroot, const DataPair& pair)
{
    ...
    if(subroot->is_leaf){
      ...
      if(subroot->children[first_larger_idx]->elements.size() >= order){ //just like insert
        split_child(subroot, first_larger_idx); //split to maintain order
      }
    }
    return;
}

...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
     ...
     */
     ...
     for(unsigned x = 0; x < frequencies.size(); x++){
       TreeNode* tmp = new TreeNode(frequencies[x]);
     ...
     while((singleQueue.size() + mergeQueue.size()) > 1){
       ...
       TreeNode* merge = new TreeNode(tmp1->freq.getFrequency() + tmp2->freq.getFrequency());
       merge->left = tmp1;
       merge->right = tmp2;
...
void HuffmanTree::decode(stringstream& ss, BinaryFileReader& bfile)
{
    ...
    while (bfile.hasBits()) {
         ...
         */
         if(bfile.getNextBit()){   //if 1 go right
           current = current->right;
         }else{   //else is 0 -> go left
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
     ...
     */
     ...
     if(!bfile.hasBits()){  //no more bits
       return NULL;
     }
     else if(bfile.getNextBit()){  //leaf node
       Frequency newf(bfile.getNextByte(),0);
       TreeNode * tmp = new TreeNode(newf);
       return tmp;
     }
     else{
       Frequency newf(0);
       TreeNode * tmp = new TreeNode(newf);
       tmp->left = readTree(bfile);
       tmp->right = readTree(bfile);
       return tmp;
     }

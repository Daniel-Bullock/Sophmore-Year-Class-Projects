...
class BinaryTree
{
    ...
    private:
        ...
        T getLeft(const Node* subRoot) const;

        T getRight(const Node* subRoot) const;

        void getPaths(std::vector<std::vector<T>>& paths,std::vector<T> curr,Node* subRoot) const;

        int sumDistances(int &summ, const Node * subTree) const;

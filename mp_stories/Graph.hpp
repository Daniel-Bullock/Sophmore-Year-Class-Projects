...
unsigned int Graph<V,E>::degree(const V & v) const {
  // TODO: Part 2
  std::list<edgeListIter> degree_ = adjList.at(v.key());
  return degree_.size();
...
V & Graph<V,E>::insertVertex(std::string key) {
  ...
  vertexMap.insert(std::pair<string, V&> (key,v));
...
void Graph<V,E>::removeVertex(const std::string & key) {
  ...
  std::vector<edgeListIter> es;

  for(auto it : adjList.at(key)){
    es.push_back(it);
  }

  unsigned size = es.size();
...
E & Graph<V,E>::insertEdge(const V & v1, const V & v2) {
  // TODO: Part 2
  E & e = *(new E(v1, v2));
  edgeListIter iter = edgeList.insert(edgeList.begin(),e);
  adjList[v1.key()].push_back(iter);
  adjList[v2.key()].push_back(iter);

...
class KDTree
{
  ...
  private:
     ...
     */
     ...
     // Helper to construct the tree
     KDTreeNode* constructorHelper(vector<Point<Dim>> newPoints, int depth)
     {
         // Base case is no remaining points
         if (newPoints.size() == 0) {
           return NULL;
         }

         // Sort point list by axis and choose median as pivot element
         unsigned median = (newPoints.size() - 1) / 2;
         Point<Dim> medPt = select(newPoints, 0, newPoints.size()-1, median, depth);

         // Smaller arrays
         auto medItr = newPoints.begin() + median;

         vector<Point<Dim>> leftPoints = {};
         leftPoints.assign(newPoints.begin(), medItr);

         vector<Point<Dim>> rightPoints = {};
         rightPoints.assign(medItr + 1, newPoints.end());

         // Create node and construct subtrees
         KDTreeNode* toReturn = new KDTreeNode(newPoints[median]);
         toReturn->left = constructorHelper(leftPoints, (depth + 1) % Dim),
         toReturn->right = constructorHelper(rightPoints, (depth + 1) % Dim);
     ...
     /*
       ...
       for help understanding how this funcitons work
     */

     unsigned partition(vector<Point<Dim>> &newPoints, unsigned left, unsigned right, unsigned pivotIndex, unsigned depth) {
       Point<Dim> pivVal = newPoints[pivotIndex];
       //Move to the end for safe keeping
       std::swap(newPoints[pivotIndex], newPoints[right]);

       unsigned saveIdx = left;
       for (unsigned i = left; i < right; i++) {
         if (smallerDimVal(newPoints[i], pivVal, depth)) {
           std::swap(newPoints[saveIdx], newPoints[i]);
           saveIdx++;
         }
       }

       //Puts in its final location
       std::swap(newPoints[right], newPoints[saveIdx]);
    ...
    Point<Dim> select(vector<Point<Dim>> &newPoints, unsigned left, unsigned right, unsigned k, unsigned depth) {
      if (left == right) {
        return newPoints[left];
      }

      unsigned pivotIndex = left + std::floor(std::rand() % (right - left + 1));
    ...
    Point<Dim> fNN(const Point<Dim>& query, KDTreeNode * subroot, int dimension) const {
      ...
      if(smallerDimVal(query, subroot->point, dimension)){
        if(subroot->left == NULL){
          best = fNN(query, subroot->right, (dimension + 1) % Dim);
        } else {
          best = fNN(query, subroot->left, (dimension + 1) % Dim);
        }
        isleft = true;
      } else {
      ...
      double cutdist = subroot->point[dimension] - query[dimension];

      cutdist = pow(cutdist,2);

      if (cutdist <= r) {
        KDTreeNode * checkpls;
        if (isleft==true) {
          checkpls = subroot->right;
        } else{
          checkpls = subroot->left;
        }

        if (checkpls != NULL) {
          Point<Dim> best2 = fNN( query,checkpls,(dimension + 1) % Dim);
          if (shouldReplace(query, best, best2)) {
            best = best2;
          }
        }
      }
